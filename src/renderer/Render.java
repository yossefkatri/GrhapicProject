...
public class Render {
    ...
    private Color calcColor(Geometry geometry,Point3D point, Ray inRay,int level){
        ...
        vector MV=new vector(point, scene.getCamera().getP0()).normalize();//get the vector from the poiint to the position of the camera.
    ...
    private Map.Entry<Geometry, Point3D> findClosesntIntersection(Ray ray) {
        ...
        Map<Geometry, Point3D> closestPoint = getClosestPoint(intersectionPoints,ray.getP00());//get the closest intersection points.
    ...
    private Ray constructRefractedRay(vector normal, Point3D point, Ray inRay) {
        ...
        no.multiply((v.dotProduct(normal) > 0)? 2 : -2);//multiply by -2 the normal depend on its position.
    ...
    private Ray constructReflectedRay(vector normal, Point3D point, Ray inRay) {
        ...
        DirectionRay=new vector(inRay.getDirection().normalize());
        vector v = new vector(normal).normalize();
        v.multiply((R.dotProduct(normal) > 0) ? 2 : -2);//multiply the normal by 2 depend on its position.
        Point3D p=new Point3D(point);
        p=p.add(v);//raise the point to prevent intersection by itself.
    ...
    private boolean occluded(LightSource light, Point3D point, Geometry geometry) {
        ...
        geometryPoint = geometryPoint.add(epsVector);//to prevent interection with itself.
    ...
    private Color scaleColor(Color c, double factor) {
        int red=(int)Math.min(Math.abs((c.getColor().getRed() * factor)), 255);
        int green= (int)Math.min(Math.abs((c.getColor().getGreen() * factor)), 255);
        int blue=(int)Math.min(Math.abs( (c.getColor().getBlue() * factor)), 255);//the max is 255
        return new Color(red,green,blue);
    }
    private Map<Geometry,Point3D> getClosestPoint(Map<Geometry,List<Point3D>> intersectionPoints,Point3D p){
    ...
    public void renderImage() {
        for(int i=0;i<imageWriter.getWidth();i++)
            for(int j=0;j<imageWriter.getHeight();j++)
            {
                ...
                Map<Geometry,List<Point3D>> intersectionPoints=getSceneRayIntersections(new Ray(ray));//find the intersection points with the ray.
                ...
                {
                     Map<Geometry,Point3D> closestPoint=getClosestPoint(intersectionPoints,scene.getCamera().getP0());//find the closest point.
            ...
            for(int j=0;j<imageWriter.getHeight();j++)
            {
                ...
                for (Ray ray: rays)
                {
                    ...
                    if (intersectionPoints.isEmpty())
                        ...
                        Map<Geometry, Point3D> closestPoint = getClosestPoint(intersectionPoints,scene.getCamera().getP0());//get the closest point
                        for (Map.Entry<Geometry, Point3D> point : closestPoint.entrySet()) {
